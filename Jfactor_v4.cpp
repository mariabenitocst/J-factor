...
double smax = 400.;
vector<double> Rsun, v0, alpha, errorAlpha, rs, errorRs, rho0, errorRho0;
vector<char> model, meanError;
ifstream E("/Users/maria/Desktop/Jfactor/MAPestimations.dat");
string line;
while ( getline(E, line))
{
	if (line.empty() || line[0] == '#')
		continue;
	double R=0., v=0., a=0., errorA=0., r=0., errorR=0., rho=0., errorRHO=0.;
	char reference[10], yes[10];
	sscanf(line.c_str(), "%s %lf %lf %lf %lf %lf %lf %lf %lf %s", reference, &R, &v, &a, &errorA, &r, &errorR, &rho, &errorRHO, yes);
	model.push_back(*reference);
	Rsun.push_back(R); v0.push_back(v);	
	alpha.push_back(a); errorAlpha.push_back(errorA);
        rs.push_back(r); errorRs.push_back(errorR);
	rho0.push_back(rho); errorRho0.push_back(errorRHO);
}

ofstream J("Jfactor.dat");
if (J.is_open())
{
	J<<"#morphology   R0   v0   alpha  error  rs  error    rho0  error  J-factor error  (GeV/cm^3)"<<endl;
	for (int i = 0; i < int(alpha.size()); i++)
	{
	double Jfactor = integrate_l(Rsun[i], smax, alpha[i], rho0[i], rs[i], bmin, bmax, lmin, lmax);
	J<<model[i]<<"   "<<Rsun[i]<<"   "<<v0[i]<<"   "<<alpha[i]<<"   "<<errorAlpha[i]<<"   "<<rs[i]<<"   "<<errorRs[i]<<"   "<<rho0[i]<<"   "<<errorRho0[i]<<"   "<<4*Jfactor<<endl;
	}
}
J.close();
