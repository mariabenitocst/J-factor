...
gsl_integration_qag(&F, bmin, bmax, 1e-3, 1e-3, limit, 0, w, &result, &error);
...
double result, error; size_t limit = sizeof(double);
gsl_integration_workspace * w = gsl_integration_workspace_alloc(10000000);
gsl_integration_qag(&G, lmin, lmax, 1e-3, 1e-3, limit, 0, w, &result, &error);
gsl_integration_workspace_free(w);
gsl_spline_free(spline);
// 1 kpc = 3.0857e21 cm 
...
/**************************************************
 ...
 I implement this part in order to reproduce the
 results in 0904.3830

***************************************************/

double solidAngle(double b, void * parameters)
{
...
double integration_solidAngle(double bmin, double bmax, double lmin, double lmax)
{
gsl_function F; F.function=&solidAngle;  F.params=&lmin;
gsl_integration_cquad_workspace* w=gsl_integration_cquad_workspace_alloc(10000);
double result, error;
size_t neval;
gsl_integration_cquad(&F, bmin, bmax, 1e-9, 1e-3, w, &result, &error, &neval);

gsl_integration_cquad_workspace_free(w);
return (lmax-lmin)*result;
}
...
int main()
{
double R0=8.5;  // (kpc)In Calore + JCAP 2015 uses R0=8.5 kpc. This is something I need to take 
	      //into account in order to be self-consistent.
double smax= 300.; // kpc			
double alpha = 1.; double rho0 = 0.3;
double rs = 20.;
double bmin = 10*M_PI/180.; 
double bmax = 20*M_PI/180.;
double lmin = 0*M_PI/180.; 
double lmax = 360*M_PI/180.;

double Jfactor =  integrate_l(R0, smax, alpha, rho0, rs, bmin, bmax, lmin, lmax);
double solid_angle=integration_solidAngle(bmin, bmax, lmin, lmax);
//double solid_angle=(lmax-lmin)*(bmax-bmin);
double adimensional_Jfactor=Jfactor/(pow(rho0, 2)*3.0857e21*8.5)/solid_angle;
cout<<adimensional_Jfactor<<endl;
return 0;
}
